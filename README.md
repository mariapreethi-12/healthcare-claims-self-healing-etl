...
The project is deliberately safe to demo: **OpenAI is optional**. Without `OPENAI_API_KEY`, a deterministic local mapper handles the included drift patterns, so the entire workflow runs offline.
...
CSV upload ─────────▶│ FastAPI ingestion endpoint   │
                  ...
                  │ deterministic mock/fallback always │
                  └─────────────────┬──────────────────┘
